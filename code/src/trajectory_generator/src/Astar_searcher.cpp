...
bool Astarpath::AstarSearch(Vector3d start_pt, Vector3d end_pt)
{
  ...
  while (!Openset.empty()) {
    ...
    if ((ros::Time::now() - t0).toSec() > max_search_time) {
      // DO NOT execute a risky bestPtr. Use retreat if clearance is low.
      ...
      if (clr < min_exec_clearance_m) {
        Vector3i retreat = pickRetreat(start_idx);
        terminatePtr = Map_Node[retreat(0)][retreat(1)][retreat(2)];
        g_reached_goal = false;
        ROS_WARN("[A*] time budget hit. best-so-far too close (clr=%.2f). Use retreat (clr=%.2f).",
                 clr, nearestObsDistM(retreat, soft_scan_cells));
        return true;
      }

      terminatePtr = bestPtr;
      g_reached_goal = false;
      ROS_WARN("[A*] time budget hit. Return best-so-far (clr=%.2f, h=%.2f).", clr, best_h);
      return true;
    }
  ...
  // openset exhausted: use retreat rather than risky bestPtr
  {
    Vector3i retreat = pickRetreat(start_idx);
    terminatePtr = Map_Node[retreat(0)][retreat(1)][retreat(2)];
    g_reached_goal = false;
    ROS_WARN("[A*] openset exhausted. Use retreat (clr=%.2f).", nearestObsDistM(retreat, soft_scan_cells));
    return true;
  }
}
